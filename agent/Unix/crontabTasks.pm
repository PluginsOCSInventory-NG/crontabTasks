...
sub addCrontab {
   ...
   my $ocsminute = $args->{MINUTE};
   my $ocshour = $args->{HOUR};
   my $ocsdom = $args->{DOM};
   my $ocsmonth = $args->{MONTH};
   my $ocsdow = $args->{DOW};
   my $ocsuser = $args->{USER};
   my $ocscommand = $args->{CRON};

   push @{$xmltags->{CRONTAB}},
   {
      MINUTE => [$ocsminute],
      HOUR => [$ocshour],
      DOM => [$ocsdom],
